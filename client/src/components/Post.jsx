...
const Post = ({ post }) => {
  ...
  const likeOrDislikeHandler = async () => {
    try {
      ...
      const res = await axios.get(`https://intsavibe.onrender.com/api/v1/post/${post._id}/${action}`, { withCredentials: true });
      if (res.data.success) {
  ...
  const commentHandler = async () => {
    try {
      const res = await axios.post(`https://intsavibe.onrender.com/api/v1/post/${post._id}/comment`, { text }, {
  ...
  const deletePostHandler = async () => {
    try {
      const res = await axios.delete(`https://intsavibe.onrender.com/api/v1/post/delete/${post?._id}`, { withCredentials: true });
      if (res.data.success) {
  ...
  const bookmarkHandler = async () => {
    try {
      const res = await axios.get(`https://intsavibe.onrender.com/api/v1/post/${post?._id}/bookmark`, { withCredentials: true });
      if (res.data.success) {
        toast.success(res.data.message);
      }
    } catch (error) {
    ...
    <div className='my-8 w-full max-w-sm mx-auto'>
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-2'>
          <Avatar>
            <AvatarImage img={post.author?.profilePicture} alt="Post_Image" />
      ...
      {comment.length > 0 && (
        <span onClick={() => { dispatch(setSelectedPost(post)); setOpen(true); }} className='cursor-pointer text-sm text-gray-400'>
